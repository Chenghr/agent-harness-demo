...
2026-09-11 更新：教师节网站场景中的定向消息、后台等待唤醒、指定子助手换模型、单文件回滚、图片版本和网站发布已补齐，新增独立的 `server/delivery/` 模块。当前行为与验证边界以 [9-缺失功能补齐与测试说明](reports/9-缺失功能补齐与测试说明.md) 为准；原有 1、2 两份核心说明保持原文。
...
后台 Agent 保持自己的模型选择，不随主 Agent 的手动切换改变；网页更新同一模型的全局配置后，后续请求使用新版本，已经发起的请求保持原参数。环境变量的 A、B 共享服务配置；网页添加的服务与模型独立保存。目前覆盖文本及函数调用场景，不迁移内部隐状态。Responses 同模型续接保留服务返回的原生 output；跨模型交接转成可见任务事实，不把供应商内部状态复制给新模型。
...
Skill 的不可信标记在教学反例中预置，加载不增加权限。能力管理已有可接真实模型的独立安全评估，新增用户工作区的独立操作审查请求和 macOS 命令沙箱；仍没有通用提示注入检测或 Skill 签名验证。测试会执行工作区内代码，因此文件路径限制不是任意代码执行时的安全边界；真实模型生成的修改也应由操作者审阅。
...
当前网页模型配置、真实工作区、权限和 Git 撤销的实现与边界见 [第 7 份说明](reports/7-模型配置、工作区权限与回滚说明.md)。
